    ...
    if(lista!=NULL &&tam>0 && punteroId!=NULL)
    {
        if(buscarLibre(lista,tam)!=-1)
        {
            do
            {
                ...
                fflush(stdin);
                if(getNumero(&nuevaBicicleta.rodado,"Ingrese rodado de la bicicleta(20/26/27.5/29): ","Error, ",20,29,3))
                {
            ...
            {
                if((agregarBicicleta(lista,tam,*punteroId,nuevaBicicleta.marca,nuevaBicicleta.idTipo
                    ,nuevaBicicleta.idColor,nuevaBicicleta.rodado))==0)
                {
                    (*punteroId)++;
                    todoOk=1;
                }
...
//        mostrarClientes(lista,tam);
        ...
        if(confirmacionModificar=='s')
        {
            do
            {
                ...
                case 1:
                    fflush(stdin);
                    if(!(getString(lista[indice].marca,20,"Ingrese la nueva marca: ","Error, ",3)))
                    {
                        printf(" ocurrio un error en la validacion\n");
                    }
                    else
...
//    mostrarClientes(lista,tam);
    ...
    do
    {
        getNumero(&idDeLaBaja,"\nIngrese el codigo del cliente que quiere dar de baja:","Error",50000,51000,3);
    }
    while(encontrarBicicletaConId(lista,tam,idDeLaBaja)!=-1&&lista[encontrarBicicletaConId(lista,tam,idDeLaBaja)].isEmpty!=1);
...
int mostrarBicicletas(eBicicleta lista[], int tam,eTipo tipos[],int tamt,eColores colores[], int tamcol)
{
    ...
    if(lista!=NULL&&tam>0)
    {
        ...
        for(int i=0; i<tam; i++)
        {
            ...
            {
                if(mostrarBicicleta(lista[i],tipos,tamt,colores,tamcol))
                {
                    flag=0;
                }
            }
